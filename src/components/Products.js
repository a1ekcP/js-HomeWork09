...
function Products(){
    ...
    return <>
        <Row xs={1} md={3} className="g-4 p-4">
            {productsArr.map(el => <Product key ={el.id} product={el} clickAdd={clickAdd} clickRemove={clickRemove}/>)}
        </Row>
        <Cart addProd={addProd} removeProd={removeProd} clickRemove={clickRemove} filteredProducts={productsArr.filter(el => el.addToCart)}/>
    </>
}
